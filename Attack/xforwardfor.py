...
def send_request(url, payload):
    ...
    try:
        subprocess.check_output(
            ["curl", "-s", "-H", f"X-Forwarded-For: {payload}", url],
    ...
    if elapsed_time >= 15:
        ...
        print(colored(f"{url}: safe with header payload {payload} (safe: {elapsed_time}s)", 'green'))
    ...
    print(f"{colored('Progress:', 'blue')} {progress}/{total_requests} ({percent_complete}%) - {remaining_hours}h:{remaining_minutes:02d}m - Elapsed time: {round(elapsed_time, 2)}s")
